...
// Exercise database with categories and suggested exercises
const exerciseDatabase = {
  chest: ["Bench Press", "Incline Press", "Decline Press", "Chest Fly", "Push-ups", "Cable Crossover", "Dumbbell Pullover", "Chest Dip"],
  back: ["Pull-ups", "Lat Pulldown", "Seated Row", "Bent Over Row", "Deadlift", "Back Extension", "Face Pull", "Shrug"],
  legs: ["Squat", "Leg Press", "Lunge", "Leg Extension", "Leg Curl", "Calf Raise", "Hip Thrust", "Romanian Deadlift"],
  shoulders: ["Shoulder Press", "Lateral Raise", "Front Raise", "Reverse Fly", "Upright Row", "Shrug", "Face Pull", "Arnold Press"],
  arms: ["Bicep Curl", "Tricep Extension", "Hammer Curl", "Skull Crusher", "Preacher Curl", "Tricep Dip", "Concentration Curl", "Close-Grip Bench"],
  core: ["Crunch", "Plank", "Russian Twist", "Leg Raise", "Mountain Climber", "Ab Rollout", "Side Plank", "Bicycle Kick"],
  cardio: ["Running", "Cycling", "Jump Rope", "Rowing", "Elliptical", "Stair Climber", "Swimming", "HIIT"]
};

// Main WorkoutLog Component
const WorkoutLog = ({ navigateTo }) => {
...
const WorkoutForm = ({ 
  ...
  useEffect(() => {
    const handleClickOutside = (event) => {
      if (suggestionsRef.current && !suggestionsRef.current.contains(event.target) && 
          exerciseInputRef.current && !exerciseInputRef.current.contains(event.target)) {
  ...
  const handleSuggestionSelect = (exercise) => {
  ...
  const handleDuplicateLastSet = (exerciseId) => {
    ...
    // Set the form values to match the last set
  ...
  return (
    <div className="workout-container bg-white dark:bg-gray-800 rounded-lg shadow-lg p-6 mb-6">
      ...
      <form onSubmit={handleAddExercise} className="mb-6">
        <label htmlFor="exercise-category" className="block mb-2 text-sm font-medium text-gray-700 dark:text-gray-300">
          Muscle Group
        </label>
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 mb-4">
          {Object.keys(exerciseDatabase).map(category => (
            <button
              key={category}
              type="button"
        ...
        <div className="flex relative">
          <input
            ...
            placeholder={selectedCategory ? `Select or type a ${selectedCategory} exercise...` : "e.g., Bench Press, Squat, Deadlift"}
            value={newExerciseName}
            onChange={(e) => {
              setNewExerciseName(e.target.value);
              if (selectedCategory) setShowSuggestions(true);
            }}
            onFocus={() => {
              if (selectedCategory) setShowSuggestions(true);
            }}
          ...
          {showSuggestions && selectedCategory && (
            <div 
              ref={suggestionsRef}
              className="absolute top-full left-0 right-14 mt-1 bg-white dark:bg-gray-700 border border-gray-300 dark:border-gray-600 rounded-lg shadow-lg z-10 max-h-60 overflow-y-auto"
            >
              {exerciseDatabase[selectedCategory]
                .filter(exercise => exercise.toLowerCase().includes(newExerciseName.toLowerCase()))
                .map((exercise, index) => (
                  <div 
                    key={index}
      ...
      {workout.exercises.length === 0 ? (
        <div className="text-center py-8 text-gray-500 dark:text-gray-400">
          ...
          <p>No exercises added yet. Start by selecting a muscle group and adding an exercise above.</p>
        </div>
      ) : (
                ...
                <div className="p-4 bg-white dark:bg-gray-800">
                  ...
                  <form onSubmit={(e) => handleAddSet(e, exercise.id)}>
                    ...
                    <div className="flex gap-2 mt-3">
                      <button
                        type="submit"
                        className="flex-1 text-white bg-primary hover:bg-primary-hover focus:ring-4 focus:outline-none focus:ring-primary/50 font-medium rounded-lg text-sm px-5 py-2.5 text-center"
                      >
                      ...
                      {exercise.sets.length > 0 && (
                        <button
                          type="button"
                          onClick={() => handleDuplicateLastSet(exercise.id)}
                          className="text-white bg-gray-600 hover:bg-gray-700 focus:ring-4 focus:outline-none focus:ring-gray-300 font-medium rounded-lg text-sm px-4 py-2.5 text-center dark:bg-gray-600 dark:hover:bg-gray-700 dark:focus:ring-gray-800"
